...
export function createAPIServer(port = 3000) {
    ...
    // Stop deployment
    app.post('/api/deployments/:name/stop', (req, res) => {
        const projectName = decodeURIComponent(req.params.name);
        
        const deployment = deploymentManager.getDeployment(projectName);
        if (!deployment) {
        ...
        if (deployment.executor) {
            try {
                if (deployment.executor.cleanup) {
                    deployment.executor.cleanup();
                } else if (deployment.executor.disconnect) {
                    deployment.executor.disconnect();
                } else if (deployment.executor.close) {
                    deployment.executor.close();
                }
            } catch (err) {
                console.error('Error cleaning up executor:', err);
            }
        }
        
        // Clear deployment sessions
        if (global.deploymentSessions) {
            global.deploymentSessions.forEach((session, key) => {
                if (key.startsWith(projectName)) {
                    global.deploymentSessions.delete(key);
                }
            });
        }
        
        // Record stopped deployment
        const duration = Date.now() - deployment.startTime;
        configManager.recordDeployment(projectName, false, {
            duration,
        ...
        deploymentManager.stopDeployment(projectName);
    ...
    app.get('/api/deployments/:name', async (req, res) => {
        ...
        if (deploymentManager.isDeploymentActive(projectName)) {
            // Set headers for SSE
            res.writeHead(200, {
            ...
            // Add this client to the existing deployment
            if (deploymentManager.addClient(projectName, res)) {
                console.log(`Client reconnected to active deployment for ${projectName}`);
                
                // Handle client disconnect
                req.on('close', () => {
                    deploymentManager.removeClient(projectName, res);
                });
                
                return; // Exit early, deployment is already running
            }
        }
        ...
        // Register deployment with manager
        const deploymentId = `${projectName}-${Date.now()}`;
        const deployment = deploymentManager.registerDeployment(projectName, deploymentId, res);

        // Helper function to send messages
        const sendMessage = (message) => {
            deploymentManager.broadcast(projectName, message);
        };
        ...
        req.on('close', () => {
            // Remove this client from deployment
            deploymentManager.removeClient(projectName, res);
            
            // Check if deployment should be stopped (no more clients)
            const activeDeployment = deploymentManager.getDeployment(projectName);
            if (!activeDeployment || activeDeployment.clients.size === 0) {
                // No more clients watching, but deployment continues in background
                console.log(`All clients disconnected from ${projectName}, deployment continues in background`);
            }
        });
        ...
        deploymentManager.broadcast(projectName, { 
            type: 'start', 
        ...
        try {
            ...
            if (project.type === 'monorepo') {
                ...
                for (const subDeployment of deploymentTargets) {
                    ...
                    if (subDeployment.deploymentSteps && subDeployment.deploymentSteps.length > 0) {
                        ...
                        if (usePersistentSession) {
                            ...
                            currentExecutor = statefulExecutor;
                            activeExecutors.push(statefulExecutor);
                            deploymentManager.setExecutor(projectName, statefulExecutor);
                            ...
                            activeExecutors.push(executor);
                            deploymentManager.setExecutor(projectName, executor);
                ...
                deploymentManager.completeDeployment(projectName, deploymentSuccess);
            ...
            // Mark deployment as complete
            deploymentManager.completeDeployment(projectName, false);
        } finally {
